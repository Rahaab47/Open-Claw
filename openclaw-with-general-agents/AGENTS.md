...
LLM model:    Gemini 3.1 Flash Lite   (free tier; gemini-3.1-flash-lite-preview is retired)
...
```bash
openclaw config set agents.defaults.model.primary "Gemini 3.1 Flash Lite"
...
1. **CLI test first**: `openclaw infer model run --prompt "hello" --model "Gemini 3.1 Flash Lite"`. Expected shape: `model.run via local` → `provider: google` → `model: Gemini 3.1 Flash Lite` → `outputs: 1` → reply text. (Older `openclaw infer "hello"` shorthand no longer parses; current surface is `infer model run`.)
